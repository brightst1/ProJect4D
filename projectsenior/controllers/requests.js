        ...
        users.findOne({'Username':req.body.Username},function(err,user){
            if(err){
                ...
                if(user.token == req.body.token){
                    requests.find({'Username':req.body.Username,'statusFlag':{$eq:1}},null,{sort:{$natural:-1}},function(err,request){
                        if(err){
                            console.log(err)
                            return res.send({err:'เกิดข้อผิดพลาด'})
                        }else if(!request){
    ...
    if(req.body.providername && req.body.token && req.body){
        providers.findOne({'Username':req.body.providername},function(err,provider){
            if(err){
                ...
                requests.find({'Providername':req.body.providername,'statusFlag':{$eq:1}},null,{sort:{$natural:-1}},function(err,request){
                    if(err){
                        console.log(err)
                        return res.send({err:'เกิดข้อผิดพลาด'})
                    }else if(!request){
